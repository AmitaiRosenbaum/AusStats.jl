...
"""
function read_cpi(;
    release::Union{Symbol, Date, AbstractString}=:latest,
    table=nothing,
    cache::Bool=true,
    ...
    return _read_catalogue_convenience(
...
"""
function read_lfs_grossflows(;
    release::Union{Symbol, Date, AbstractString}=:latest,
    cube::Union{Nothing, AbstractString}="gross flows",
    cache::Bool=true,
    cache_parsed::Bool=true,
    refresh::Bool=false,
    family::Symbol=:auto,
)
    return _read_cube_convenience(
...
"""
function read_lfs_cube(;
    cube::Union{Nothing, AbstractString}=nothing,
    release::Union{Symbol, Date, AbstractString}=:latest,
    cache::Bool=true,
    cache_parsed::Bool=true,
    refresh::Bool=false,
...
function _read_catalogue_convenience(
...
function _read_cube_convenience(
    label::AbstractString,
    cat_no::AbstractString;
    cube,
    release,
...
function _throw_convenience_error(label::AbstractString, cat_no::AbstractString, error)
    ...
    throw(
        ArgumentError(
            "could not read $label catalogue `$cat_no`: $message. Try `files(\"$cat_no\"; refresh=true)` to inspect current ABS downloads, or use the generic `read_abs`/`read_cube` APIs with an explicit file selector.",
        ),
    )
end
