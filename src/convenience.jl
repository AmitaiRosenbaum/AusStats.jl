...
"""
    read_cpi(; release=:latest, table=nothing, cache=true, refresh=false, tidy=true)

Read Consumer Price Index, Australia (`6401.0`) using [`read_abs`](@ref).
`table` is passed to the generic `tables` selector.
"""
function read_cpi(; release::Union{Symbol,Date,AbstractString}=:latest, table=nothing, cache::Bool=true, refresh::Bool=false, tidy::Bool=true)
    return _read_catalogue_convenience("Consumer Price Index", _CPI_CAT_NO; release, table, cache, refresh, tidy)
end

"""
    read_awe(; release=:latest, table=nothing, cache=true, refresh=false, tidy=true)

Read Average Weekly Earnings, Australia (`6302.0`) using [`read_abs`](@ref).
`table` is passed to the generic `tables` selector.
"""
function read_awe(; release::Union{Symbol,Date,AbstractString}=:latest, table=nothing, cache::Bool=true, refresh::Bool=false, tidy::Bool=true)
    return _read_catalogue_convenience("Average Weekly Earnings", _AWE_CAT_NO; release, table, cache, refresh, tidy)
end

"""
    read_erp(; release=:latest, table=nothing, cache=true, refresh=false, tidy=true)
...
`tables` selector.
"""
function read_erp(; release::Union{Symbol,Date,AbstractString}=:latest, table=nothing, cache::Bool=true, refresh::Bool=false, tidy::Bool=true)
    return _read_catalogue_convenience("Estimated Resident Population", _ERP_CAT_NO; release, table, cache, refresh, tidy)
end

"""
    read_job_mobility(; release=:latest, table=nothing, cache=true, refresh=false, tidy=true)

Read Job Mobility, Australia (`6226.0`) using [`read_abs`](@ref). `table` is
passed to the generic `tables` selector.
"""
function read_job_mobility(; release::Union{Symbol,Date,AbstractString}=:latest, table=nothing, cache::Bool=true, refresh::Bool=false, tidy::Bool=true)
    return _read_catalogue_convenience("Job Mobility", _JOB_MOBILITY_CAT_NO; release, table, cache, refresh, tidy)
end

"""
    read_payrolls(; release=:latest, table=nothing, cache=true, refresh=false, tidy=true)
...
"""
function read_payrolls(; release::Union{Symbol,Date,AbstractString}=:latest, table=nothing, cache::Bool=true, refresh::Bool=false, tidy::Bool=true)
    return _read_catalogue_convenience("Weekly Payroll Jobs and Wages", _PAYROLLS_CAT_NO; release, table, cache, refresh, tidy)
end
...
function _read_catalogue_convenience(label::AbstractString, cat_no::AbstractString; release, table, cache::Bool, refresh::Bool, tidy::Bool)
    ...
    try
        return read_abs(cat_no; tables=table, release, tidy, cache)
    catch error
