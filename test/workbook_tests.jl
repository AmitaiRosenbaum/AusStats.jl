...
@testset "Workbook parser helper edge cases" begin
    ...
    @test AusStats._column_names(Any[missing, "", "A B", "A B"]) ==
        [:Column1, :Column2, :A_B, :A_B_2]
    ...
    fallback_rows = Any[Any["A1234567X", "Description"], Any["2024", 10]]
    @test AusStats._series_id_for_column(fallback_rows, 1, 2, Dict{String, Int}()) ==
        "A1234567X"
    @test AusStats._series_id_for_column(fallback_rows, 2, 2, Dict{String, Int}()) ==
        "Column 2"
    @test AusStats._series_id_for_row(Any[]) == ""
    @test AusStats._series_id_for_row(Any["Plain series", 10]) == "Plain series"
    @test AusStats._series_name_for_row(Any["Plain series", 10], Any["Name", "Value"]) ==
        "Plain series"
    @test ismissing(
        AusStats._series_name_for_row(
            Any["A1234567X", Date(2024, 1, 1), 10], Any["ID", "Date", "Value"]
        ),
    )
    ...
    @test AusStats._table_title_score("Tables 2b to 9b. All quarterly series") >
        AusStats._table_title_score("Short")
    ...
    @test AusStats._infer_release_value(
        Any[Any["Release Date", ""], Any["Released: April 2024"]]
    ) == "apr-2024"
    @test isempty(
        AusStats._tidy_sheet_time_across(
            Any[Any["No dates"], Any["Still no dates"]], "NoDates"
        ),
    )
    @test AusStats._first_matching_text(
        Any["none", "Catalogue 1234.5"], r"[0-9]{4}\.[0-9]"
    ) == "1234.5"
