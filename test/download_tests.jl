@testset "Download and discovery edge cases" begin
    ...
    rows = AusStats._file_rows_dataframe(
    ...
    @test AusStats._archive_links(doc, seed) == [
        "https://www.abs.gov.au/statistics/labour/employment-and-unemployment/labour-force-australia/archive",
    ]
    @test AusStats._release_links(doc, seed) == [
        "https://www.abs.gov.au/statistics/labour/employment-and-unemployment/labour-force-australia/may-2026",
    ]

    @test AusStats._absolute_url(" HTTP://example.test/file.xlsx ") ==
        "HTTP://example.test/file.xlsx"
    @test AusStats._absolute_url("//example.test/file.xlsx") ==
        "https://example.test/file.xlsx"
    @test AusStats._absolute_url("relative/file.xlsx"; base="https://example.test/base/") ==
        "https://example.test/base/relative/file.xlsx"
    @test AusStats._safe_filename(" ?? ") == "download"
    @test AusStats._url_filename("https://example.test/") == "download"
    @test AusStats._file_type("https://example.test/download", "Detailed data cube") ==
        "cube"
    ...
    releases_df = AusStats._release_rows_dataframe([
        AusStats._release_row(;
            cat_no="9999.0",
            title="Bad",
            release_date=Date(2026, 2, 1),
            release_url="https://example.test/bad",
        ),
    ])
    AusStats._write_release_index("9999.0", releases_df)
    @test AusStats._read_release_index("9999.0").release_date == [Date(2026, 2, 1)]
    write(
        AusStats._release_index_path("9999.0"),
        """[{"cat_no":"9999.0","title":"Bad date","release_date":"feb-2026","release_url":"https://example.test/bad"}]""",
    )
    @test nrow(AusStats._read_release_index("9999.0")) == 0

    @test_throws ArgumentError AusStats._select_file(
        "6202.0"; file="definitely missing", cube=false
    )
    @test_throws ArgumentError AusStats._select_file(
        "6202.0"; release="definitely missing", cube=false
    )
...
@testset "Discovery helper edge cases" begin
    ...
    @test AusStats._release_page_from_file_url("https://example.test/path/file.xlsx") ==
        "https://example.test/path"
    @test AusStats._release_page_from_file_url(
        "https://example.test/path/may-2026/file.xlsx"
    ) == "https://example.test/path/may-2026"
    @test AusStats._release_date_from_url("https://example.test/path/may-2026/file.xlsx") ==
        Date(2026, 5, 1)
    ...
    @test AusStats._looks_like_release_url(
        seed.page_url * "/release/may-2026/details", seed
    )
    @test !AusStats._looks_like_release_url("https://example.test/may-2026", seed)
    @test occursin(
        "no releases are known",
        AusStats._missing_release_message("9999.0", Date(2026, 1, 1), Date[]),
    )
    @test occursin(
        "2026-02-01",
        AusStats._missing_release_message(
            "9999.0", Date(2026, 1, 15), [Date(2026, 2, 1), Date(2027, 1, 1)]
        ),
    )
    ...
    try
        ...
        release_rows = AusStats._release_rows_dataframe([
            AusStats._release_row(;
                ...
                release_url=base * "/release-file-page",
            ),
        ])
        ...
        release_files = AusStats._files_for_release(
            "6202.0", Date(2026, 9, 1); refresh=false, strict=true
        )
        ...
        no_html_rows = AusStats._release_rows_dataframe([
            AusStats._release_row(;
                ...
                release_url=base * "/broken",
            ),
        ])
        AusStats._write_release_index("6202.0", no_html_rows)
        @test nrow(
            AusStats._files_for_release(
                "6202.0", Date(2026, 10, 1); refresh=false, strict=false
            ),
        ) == 0
    finally
        close(server)
